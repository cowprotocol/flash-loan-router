...
/// @notice A contract that tracks flash loan takeouts and paybacks for borrowers.
contract FlashLoanTracker {
    ...
    // borrower -> user -> token -> amount
    mapping(address => mapping(address => mapping(IERC20 => uint256))) public open;
    ...
    {
        if (open[_borrower][_user][_token] != 0) {
            revert FlashLoanTrackerError.AlreadyTakenOut();
        }

        open[_borrower][_user][_token] = _amount;
        _token.safeTransferFrom(_borrower, _user, _amount);
    }

    function payBack(address _borrower, address _user, IERC20 _token) external onlySettlementContract {
        uint256 _amount = open[_borrower][_user][_token];
        if (_amount == 0) {
            revert FlashLoanTrackerError.NotTakenOut();
        }

        open[_borrower][_user][_token] = 0;
        _token.safeTransferFrom(_user, _borrower, _amount);
    }
}
