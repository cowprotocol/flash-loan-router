...
contract DeployERC3156Borrower is Script {
    ...
    function deployERC3156Borrower(FlashLoanRouter flashLoanRouter) internal returns (ERC3156Borrower borrower) {
        ...
        address expectedAddress = vm.computeCreate2Address(
            Constants.SALT, keccak256(abi.encodePacked(type(ERC3156Borrower).creationCode, abi.encode(flashLoanRouter)))
        );
        ...
        if (expectedAddress.code.length == 0) {
            vm.broadcast();
            borrower = new ERC3156Borrower{salt: Constants.SALT}(flashLoanRouter);
            console.log("ERC3156Borrower has been deployed at:", address(borrower));
        } else {
            borrower = ERC3156Borrower(expectedAddress);
            console.log("ERC3156Borrower was already deployed at:", address(borrower));
        }
    }
}
