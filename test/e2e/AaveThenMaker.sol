...
contract E2eAaveThenMaker is Test {
    ...
    IFlashLoanSolverWrapper private aaveSolverWrapper;
    IFlashLoanSolverWrapper private makerSolverWrapper;
    ...
    function setUp() external {
        ...
        aaveSolverWrapper = AaveSetup.prepareSolverWrapper(vm, router, solver);
        makerSolverWrapper = MakerSetup.prepareSolverWrapper(vm, router, solver);
    }
    ...
    function test_settleWithFlashLoans() external {
        ...
        // At the time of the settlement, each solver wrapper holds the
        // respective token.
        interactionsWithFlashLoan[0] = CowProtocolInteraction.pushBalanceToAccumulator(
            tokenBalanceAccumulator, Constants.WETH, address(aaveSolverWrapper)
        );
        interactionsWithFlashLoan[1] = CowProtocolInteraction.pushBalanceToAccumulator(
            tokenBalanceAccumulator, Constants.DAI, address(makerSolverWrapper)
        );
        expectedBalances[0] =
            TokenBalanceAccumulator.Balance(Constants.WETH, address(aaveSolverWrapper), aaveLoanedAmount);
        expectedBalances[1] =
            TokenBalanceAccumulator.Balance(Constants.DAI, address(makerSolverWrapper), makerLoanedAmount);
        ...
        interactionsWithFlashLoan[2] = CowProtocolInteraction.transferFrom(
            Constants.WETH, address(aaveSolverWrapper), address(Constants.SETTLEMENT_CONTRACT), aaveLoanedAmount
        );
        interactionsWithFlashLoan[3] = CowProtocolInteraction.transferFrom(
            Constants.DAI, address(makerSolverWrapper), address(Constants.SETTLEMENT_CONTRACT), makerLoanedAmount
        );
        ...
        // Prepare the wrappers for enabling repayment of the loan.
        interactionsWithFlashLoan[6] = CowProtocolInteraction.wrapperApprove(
            aaveSolverWrapper, Constants.WETH, address(AaveSetup.WETH_POOL), aaveAmountWithFee
        );
        interactionsWithFlashLoan[7] = CowProtocolInteraction.wrapperApprove(
            makerSolverWrapper, Constants.DAI, address(MakerSetup.FLASH_LOAN_CONTRACT), makerAmountWithFee
        );

        // Send back the funds to the solver wrapper for repaying the loan.
        interactionsWithFlashLoan[8] =
            CowProtocolInteraction.transfer(Constants.WETH, address(aaveSolverWrapper), aaveAmountWithFee);
        interactionsWithFlashLoan[9] =
            CowProtocolInteraction.transfer(Constants.DAI, address(makerSolverWrapper), makerAmountWithFee);
